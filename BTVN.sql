...
as full_name, e.hire_date, dep.department_name 
FROM employees e JOIN departments dep
...
--------------------------------------------CHO VÍ DỤ---------------------------------------------------------------------------
-- câu 1: 
--Tạo view 1:  Hiển thị tên và mức lương của những nhân viên làm 25 năm trở lên 
...
-- Tạo view 2 : Hiển thị thông tin cá nhân của tất cả nhân viên với department_id >= 5
...
-- câu 2: 
--Procedure 1: tạo một pro_Search_Name(Fistname)
...
AS
BEGIN
    SELECT employee_id, first_name + ' ' + last_name AS fullname, email, phone_number, hire_date, job_id, salary, manager_id, department_id 
    FROM employees 
    WHERE first_name LIKE '%' + @firstname + '%';
END;

EXEC pro_search_name @firstname = 'lex';


-- Procedure 2: tạo một thủ tục có tên proc_Salary để xét thưởng cuối năm như sau 
...
BEGIN
    SELECT e.employee_id, e.first_name, e.last_name, e.hire_date, 
           CASE 
               WHEN DATEDIFF(day, e.hire_date, GETDATE()) / 365 >= 35 THEN e.salary + 20000000
               WHEN DATEDIFF(day, e.hire_date, GETDATE()) / 365 >= 30 THEN e.salary + 15000000
               WHEN DATEDIFF(day, e.hire_date, GETDATE()) / 365 >= 25 THEN e.salary + 10000000
               ELSE e.salary + 5000000
           END AS 'Luong Thuong Cuoi Nam'
    FROM employees e;
END
